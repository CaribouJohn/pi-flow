...
| 0002 | Read-only board | Electrobun tray app: **supervisor + viewer** — supervises the `flowd daemon` child and renders the engine's `World` as `NEEDS YOU / RUNNING / DONE` (click-through, daemon liveness/controls, slice-detail trust view, tray badge + OS notify on new NEEDS YOU). Single-repo, read-only. Scaffolds a fresh Electrobun shell applying the Hiss RPC seam + lessons. | ready to slice ([#103](https://github.com/CaribouJohn/pi-flow/issues/103) → [PRD](./0002-read-only-board.md), [ADR-0039](../adr/0039-dashboard-topology.md)) — first **pixels** track |
