...
project profile** (by convention `docs/agents/flow-profile.md`), not in the skill. The
...
2. Run `/setup-flow` (the companion skill) — or by hand: add a **flow profile** at
   `docs/agents/flow-profile.md` describing *your* tracker access, role↔label mapping,
