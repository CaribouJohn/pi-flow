...
export function parseArgs(argv: string[]): ParsedArgs {
  ...
  return { command, track, issue, prd, config };
}

/** What the CLI should do: report a usage error, run a track, or run a plan. */
export type RunPlan =
...
export function planInvocation(argv: string[]): RunPlan {
  const { command, track, issue, prd, config } = parseArgs(argv);
